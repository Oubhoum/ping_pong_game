...
win.title("Ping Pong Game") #set the title of the window
win.bgcolor("black") #set the background color of the window
win.setup(width=800, height=600) #set the width and height of the window
win.tracer(0) #stops the wondow from updating automaclly

# madrab1
madrab1 = turtle.Turtle() #initialized turtle object(shape)
...
def madrab1_down():
    y = madrab1.ycor()
    y -= 20 # set the y to decrese be 20
    madrab1.sety(y)
...
win.listen()
win.onkeypress(madrab1_up, "w") # tell the window to expect keyboard input
win.onkeypress(madrab1_down, "s") # when perssing w the function madrab1_up in invoked
...
#main game loop
while True:
    win.update() #update the screen everytime the loop run
    ...
    # border check , top border +300px, bottom border -300px, ball is 200px
    ...
    if ball.xcor() > 390: # if ball is att right border
    ...
    if ball.xcor() < -390:  # if ball is att left border
        ball.goto(0, 0)
        ball.dx *= -1
        score2 += 1
        score.clear()
